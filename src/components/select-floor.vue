  ...
  <div class="page-wrapper">
    ...
    <div class="page">
      <floorSсhemeSVG
        class="floorSсheme"
        ref="floorSсheme"
        @mouseover="updateFloorByHover($event)"
      />
...
export default {
  ...
  mounted() {
    ...
    this.possibleFloorElArr.sort(
      (a, b) => a.dataset.floorNumber - b.dataset.floorNumber
    );
  ...
  methods: {
    getFloorElByNumber: function (number) {
      ...
      this.svgFloorEls.forEach((el) => {
        if (
          el.dataset.floorNumber == number &&
          el.dataset.buildingNumber == this.selectedBuilding
        ) {
          result = el;
        }
      });
    ...
    incrementFloor() {
      let oldValueIndex = this.possibleFloorArr.findIndex(
        (currentValue) => currentValue == this.selectedFloor
      );
      if (oldValueIndex < this.possibleFloorArr.length - 1)
        this.$store.commit(
          "updateSelectedFloor",
          this.possibleFloorArr[oldValueIndex + 1]
        );
      else return;
    },
    decrementFloor() {
  ...
  watch: {
    selectedFloor: function () {
      this.svgFloorEls.forEach((el) => el.setAttribute("fill", "transparent"));
