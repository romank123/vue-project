<template>
  <div id="app">
    ...
    <select-area
      ...
      v-if="step.current === 4"
    />
    <!-- выберете офис на конкретном этаже -->
    <select-options
      @incrementStep="incrementStep"
      @decrementStep="decrementStep"
      v-if="step.current === 5"
    />
    <!-- выберете дополнительные опции для офиса-->
    <select-walls
      @incrementStep="incrementStep"
      @decrementStep="decrementStep"
      v-if="step.current === 6"
    />
    <!-- варианты отделки: стены -->
    <select-flooring
      @incrementStep="incrementStep"
      @decrementStep="decrementStep"
      v-if="step.current === 7"
    />
    <!-- варианты отделки: напольное покрытие -->
    <select-ceiling
      @incrementStep="incrementStep"
      @decrementStep="decrementStep"
      v-if="step.current === 8"
    />
    <!-- варианты отделки: потолок -->
    ...
    <select-partitions
      @incrementStep="incrementStep"
      @decrementStep="decrementStep"
      v-if="step.current === 10"
    />
    <!-- варинты отделки: перегородки -->
    <send-request v-if="step.current === 11" />
    <!-- отправка заявки -->
  </div>
</template>
...
import selectFlooring from "./components/select-flooring.vue";
import selectCeiling from "./components/select-ceiling.vue";
import selectLayoutOptions from "./components/select-layoutOptions.vue";
import selectPartitions from "./components/select-partitions.vue";
import sendRequest from "./components/send-request.vue";
